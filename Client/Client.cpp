...
void ClientLoadingScreen::Update(float dt)
{
...
void ClientComponent::ChangeRenderPath(RENDERPATH path)
{
	switch (path)
	{
	case ClientComponent::RENDERPATH_DEFAULT:
		...
		break;
	case ClientComponent::RENDERPATH_PATHTRACING:
...
void ClientComponent::ResizeBuffers()
{
...
void ClientComponent::ResizeLayout()
{
...
void ClientComponent::Load()
{
	...
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { pointLightTex = wi::resourcemanager::Load("../Assets/pointlight.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { spotLightTex = wi::resourcemanager::Load("../Assets/spotlight.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { dirLightTex = wi::resourcemanager::Load("../Assets/directional_light.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { areaLightTex = wi::resourcemanager::Load("../Assets/arealight.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { decalTex = wi::resourcemanager::Load("../Assets/decal.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { forceFieldTex = wi::resourcemanager::Load("../Assets/forcefield.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { emitterTex = wi::resourcemanager::Load("../Assets/emitter.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { hairTex = wi::resourcemanager::Load("../Assets/hair.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { cameraTex = wi::resourcemanager::Load("../Assets/camera.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { armatureTex = wi::resourcemanager::Load("../Assets/armature.dds"); });
	wi::jobsystem::Execute(ctx, [this](wi::jobsystem::JobArgs args) { soundTex = wi::resourcemanager::Load("../Assets/sound.dds"); });
	...
	modelButton.OnClick([&](wi::gui::EventArgs args) {
		...
		params.description = "vEngine Assets (.vasset, .gltf, .glb)";
		params.extensions.push_back("vasset");
...
void ClientComponent::PreUpdate()
{
...
{
	wi::profiler::range_id profrange = wi::profiler::BeginRangeCPU("Client Update");
				...
				{	
					if (translator.selected.empty() && object->GetRenderTypes() & wi::enums::RENDERTYPE_WATER)
					{
						if (wi::input::Down(wi::input::MOUSE_BUTTON_LEFT))
						{
							...
							scene.PutWaterRipple("../Assets/ripple.png", hovered.position);
						}
					}
					else if (decalWnd.placementCheckBox.GetCheck() && wi::input::Press(wi::input::MOUSE_BUTTON_LEFT))
					{
						...
						Entity entity = scene.Entity_CreateDecal("editorDecal", (decalselector == 0 ? "../Assets/leaf.dds" : "../Assets/blood1.png"));
...
void ClientComponent::PostUpdate()
{
	RenderPath2D::PostUpdate();

	renderPath->PostUpdate();
}
void ClientComponent::Render() const
{
	Scene& scene = wi::scene::GetScene();
	...
	if(renderPath->GetDepthStencil() != nullptr && !translator.selected.empty())
	{
		...
		device->EventBegin("Client - Selection Outline Mask", cmd);
...
void ClientComponent::Compose(CommandList cmd) const
{
	...
	{
		GraphicsDevice* device = wi::graphics::GetDevice();
		device->EventBegin("Client - Selection Outline", cmd);
...
void ClientComponent::PushToSceneGraphView(wi::ecs::Entity entity, int level)
{
...
void ClientComponent::RefreshSceneGraphView()
{
	const Scene& scene = wi::scene::GetScene();
...
void ClientComponent::ClearSelected()
{
	translator.selected.clear();
}
void ClientComponent::AddSelected(Entity entity)
{
	wi::scene::PickResult res;
	res.entity = entity;
	AddSelected(res);
}
void ClientComponent::AddSelected(const PickResult& picked)
{
	bool removal = false;
...
bool ClientComponent::IsSelected(Entity entity) const
{
	for (auto& x : translator.selected)
	{
		if (x.entity == entity)
...
void ClientComponent::ResetHistory()
{
	historyPos = -1;
	history.clear();
}
wi::Archive& ClientComponent::AdvanceHistory()
{
	historyPos++;
...
void ClientComponent::ConsumeHistoryOperation(bool undo)
{
