...
vEngine.math.lerp = function(minValue, maxValue, lerpRate)
    ...
    assert(not minValue or (type(minValue) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid minValue"))
    assert(not maxValue or (type(maxValue) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid maxValue"))
    assert(not lerpRate or (type(lerpRate) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid lerpRate"))
...
vEngine.math.clamp = function(value, minValue, maxValue)
    ...
    assert(not value or (type(value) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid value"))
    assert(not minValue or (type(minValue) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid minValue"))
    assert(not maxValue or (type(maxValue) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid maxValue"))
...
vEngine.math.round = function(value, decimals)
    ...
    assert(not decimals or (type(decimals) < 0), vEngine.prepareDebugMessage(API, "Error", "Invalid decimals"))
...
vEngine.math.tickProgress = function(tick, duration)
    ...
    assert(not tick or (type(tick) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid tick"))
...
vEngine.math.angle.shortTarget = function(initialAngle, finalAngle)
    ...
    assert(not initialAngle or (type(initialAngle) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid initialAngle"))
    assert(not finalAngle or (type(finalAngle) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid finalAngle"))
...
vEngine.math.angle.rotationAngle2D = function(startX, startY, endX, endY) 
    ...
    assert(not startX or (type(startX) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid startX"))
    assert(not startY or (type(startY) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid startY"))
    assert(not endX or (type(endX) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid endX"))
    assert(not endY or (type(endY) ~= "number"), vEngine.prepareDebugMessage(API, "Error", "Invalid endY"))
