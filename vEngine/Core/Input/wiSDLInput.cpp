...
namespace wi::input::sdlinput
{
    ...
    int to_vEngine(const SDL_Scancode &key);
    void controller_to_vEngine(uint32_t *current, Uint8 button, bool pressed);
        ...
        if (ret < 0) {
            ...
            for (const SDL_Event &event : events) {
                switch (event.type) {
                    ...
                    {
                        int converted = to_vEngine(event.key.keysym.scancode);
                        if (converted >= 0) {
        ...
        for(auto& controller : controllers){
            if(controller.controller){
                ...
                for(int btn=SDL_CONTROLLER_BUTTON_INVALID; btn<SDL_CONTROLLER_BUTTON_MAX; btn++){
                    controller_to_vEngine(&controller.state.buttons,
                        btn,
